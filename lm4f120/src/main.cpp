...
class Button : public Actor, public ValueFlow<bool> {
 ...
 public:
  ...
  static void isrButton1() {
    if (_button1) _button1->newValue(digitalRead(_button1->_pin) == 0);
  }

  static void isrButton2() {
    if (_button2) _button2->newValue(digitalRead(_button2->_pin) == 0);
  }

  void init() {
    pinMode(_pin, INPUT_PULLUP);
    if (_pin == PF_4) attachInterrupt(_pin, isrButton1, CHANGE);
    if (_pin == PF_0) attachInterrupt(_pin, isrButton2, CHANGE);
  };
};
